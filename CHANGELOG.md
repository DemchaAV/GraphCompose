...
- **A table cell's `lineSpacing` is space between its lines.** The layout sized a row with
  ...
  weekly schedule's event cells drew their lines touching inside rows sized to hold them
  apart. Both renderers now put the spacing between the lines, so the text fills the height
  the row was sized for; the DOCX export writes such a cell as a paragraph per line with the
  spacing after each but the last, and in LibreOffice its lines stand the same distance apart
  as on the page. A cell without line spacing is drawn and written as before. Two committed
  previews move — `invoice-consulting-v2.pdf` and `weekly-schedule.pdf` — and are re-rendered.
