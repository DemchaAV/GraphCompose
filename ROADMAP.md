...
- **Next.js showcase site** &mdash; shipped in v1.6.8 as the official GitHub Pages deploy target. Replaces the legacy `docs/index.html` landing with a Next.js 14 App Router static-export under `site/`, deployed via `.github/workflows/deploy-site.yml`. Unified Gallery surfaces all 53 generated catalogue PDFs with category filters + search + native browser PDF preview, the Playground hosts the live Monaco editor with three real GraphCompose snippets (hello / invoice / cv), and `cut-release.ps1` keeps the site in lockstep with future versions automatically.
