...
| Writing direction inside a **table cell** | ❌ a cell's text carries no direction — Hebrew draws reversed and Arabic unjoined; compose such content as a paragraph (the Hebrew invoice example builds its line items from rows for this reason) | ❌ same limit | ⚠️ a cell holding a **paragraph** carries that paragraph's direction and alignment (`writeCellNode` → `applyParagraphProperties`), which is what a row's cells hold. A cell given plain **strings** (`table.row("…")`) still carries neither, and column order is not mirrored |
