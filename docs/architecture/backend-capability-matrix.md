...
| Bidi mirroring — paired punctuation in a reordered line (UAX #9 L4) | ✅ `BidiMirroring` swaps at the PDF seam, so a parenthesis faces what it encloses; the mirrored set is document punctuation rather than the whole Unicode table | ⚠️ `PptxParagraphFragmentRenderHandler` + `BidiMirroring` — measured on PowerPoint, it places a neutral from the declared direction but does not go on to mirror it, so the swap is made before the text is handed over. **That measurement is the load-bearing assumption of this row**, not a property of the format: a consumer that does apply L4 (any ICU-backed viewer) mirrors the character a second time and draws issue [#545](https://github.com/DemchaAV/GraphCompose/issues/545) again, with the stored text wrong as well. Only in PowerPoint is the result correct. The swap is keyed to the span's direction, so an inline chip — one fill, unsplittable, given its first character's direction whole — is exempt rather than swapped over an interior at the opposite level. The stored run holds the mirrored character, including `<` and `>` | n/a — Word mirrors from the logical text it is given |
