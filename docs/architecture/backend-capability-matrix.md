...
| Bidi mirroring — paired punctuation in a reordered line (UAX #9 L4) | ✅ `BidiMirroring` swaps at the PDF seam, so a parenthesis faces what it encloses; the mirrored set is document punctuation rather than the whole Unicode table. An atomic chip that mixes directions is resolved level by level (`BidiVisualOrder`) rather than reversed whole, so its left-to-right interior keeps reading the way it was typed | ⚠️ `PptxParagraphFragmentRenderHandler` + `BidiMirroring` — measured on PowerPoint, it places a neutral from the declared direction but does not go on to mirror it, so the swap is made before the text is handed over. **That measurement is the load-bearing assumption of this row**, not a property of the format: a consumer that does apply L4 (any ICU-backed viewer) mirrors the character a second time and draws issue [#545](https://github.com/DemchaAV/GraphCompose/issues/545) again, with the stored text wrong as well. Only in PowerPoint is the result correct. The swap is keyed to the run, and only a run that is **uniformly** right-to-left gets it (`BidiVisualOrder.mirrorRightToLeftLevels`): that is the one PowerPoint reverses for itself, so the glyph has to be swapped for it, while a run mixing levels comes out right from the text exactly as typed and pre-swapping it drew `)a > b(` on a slide. A chip is the span that can hold both, since one rounded fill means the wrapper cannot split it — so `renderChip` splits its **text** instead, one frame per directional run placed in visual order (`BidiVisualOrder.visualRuns`), leaving each frame single-level and PowerPoint nothing to re-resolve. Handed over whole it was re-resolved as a fragment without the line around it, and the deck disagreed with the PDF about where a chip's Latin sat. The runs are measured on the joined forms, the width the fill was sized with; measured unshaped, an Arabic chip's last run ended past its own background. The stored run holds the mirrored character where the swap applies, including `<` and `>` | n/a — Word mirrors from the logical text it is given |
| Text extraction fidelity of reordered/shaped text | ✅ the font's `ToUnicode` map names the letters an author wrote rather than the shaped forms (`PdfShapedGlyphUnicode`, applied after subsetting and before encryption), and each reversed run carries its written text as `ActualText` (`PdfActualText`) so the reading order is in the file rather than re-derived; per-glyph text inside a section is reachable through the font's map rather than plain extraction | ⚠️ the runs are stored as written except for mirrored punctuation, so copying a right-to-left line out of a slide yields swapped brackets; a chip's letters stay in logical order, its right-to-left-level punctuation mirrored (see the mirroring row) | n/a — the runs are stored as written |
| Font portability — does a file carry the family it drew with | ✅ every embedded family is subset into the file (`PdfFontLoader`), so a reader needs nothing installed | ✅ `PptxRenderEnvironment.embedBundledFontsUsed` + `PptxEmbeddedFont` — a registered family and a bundled one the deck drew with are both carried. Whole-font, no subsetting — the **bundled** path narrows that to the faces a run actually drew, so a five-script deck grows from 27 KB to about 1.4 MB rather than carrying weights no slide contains; a **registered** family is still taken at family granularity, all four faces. `PptxFixedLayoutBackend.Builder.embedBundledFonts(false)` declines the bundled half (a registered family travels regardless), and a declined or failed family is warned about by name. Standard-14 names carry no binary and map to metric-compatible viewer replacements (Helvetica to Arial, and so on) rather than being embedded | ❌ `w:rFonts` names the family and nothing is carried, so a reader without it installed substitutes — Word's own `w:embedRegular` + font table is unused |
| Writing direction inside a **table cell** (`DocumentTableStyle.direction`) — the row grades the plain-**string** cell (`table.row("…")`). A cell holding a composed **paragraph** is a paragraph everywhere and takes the row above. | ✅ `TableLayoutSupport` answers the declared direction against the cell's own text — `AUTO` per cell, never per line — and `PdfTableRowFragmentRenderHandler` shapes the letters, draws them in display order, and marks the line with its written text as `ActualText`. Auto columns are measured on the joined forms, so the column fits what is drawn | ✅ `PptxTableRowFragmentRenderHandler` declares the direction on the cell's frame and hands PowerPoint the text as written, mirroring nothing and keeping every Unicode formatting control (`PdfFont.sanitizeForLogicalTextExport`) — the joining controls because PowerPoint shapes, the direction marks because it also resolves the order, which is more than the paragraph path leaves it. This is the opposite of the paragraph rows above and for a stated reason: a paragraph arrives as one frame per span, so no frame holds a bracket pair for PowerPoint to resolve and the swap has to be made first, whereas a cell is one frame holding a whole line — the input PowerPoint's own implementation is complete for. Measured on a slide: pre-mirroring a cell drew `(2026)` as `)2026(`. A copy out of a cell therefore carries the brackets as typed, which the paragraph path cannot promise | ⚠️ `DocxSemanticBackend.writeCellContent` writes `w:bidi` on the cell's paragraph and hands Word the text untouched, for its own bidi engine to order and join. Cell **alignment** is written on no cell, right-to-left or not, so an explicit `textAnchor` does not reach Word — the direction-derived edge does, because Word's own default is the start of the flow. Column order is not mirrored (`w:tblPr/w:bidiVisual` is unwritten) |
