...
| Writing direction inside a **table cell** — the row grades the plain-**string** cell (`table.row("…")`), which is the path that carries nothing on any backend. A cell holding a composed **paragraph** is a paragraph everywhere and takes the row above. | ❌ a string cell's text carries no direction — Hebrew draws reversed and Arabic unjoined; compose such content as a paragraph (the Hebrew invoice example builds its line items from rows for this reason) | ❌ same limit | ⚠️ a string cell carries neither direction nor alignment, and column order is not mirrored (`w:tblPr/w:bidiVisual` is unwritten). A cell holding a paragraph does carry both, through `writeCellNode` → `applyParagraphProperties` — which is what a row's cells hold, so an invoice built from rows is covered |
