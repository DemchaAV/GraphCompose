...
| Horizontal divider | Done | `DividerBuilder` is the canonical convenience over a thin `ShapeNode`. The "Partial" earlier note reflected the implementation choice (alias rather than dedicated node), not a missing feature — every divider configuration that authors reach for is supported. |
...
| Transform (rotate / scale) | Done | `ShapeContainerBuilder`, `ShapeBuilder`, `LineBuilder`, `EllipseBuilder`, `ImageBuilder`, and `BarcodeBuilder` all implement `Transformable<T>` via the `rotate(degrees)` / `scale(uniform)` / `scale(sx, sy)` mixin. PDF backend wraps each transformed leaf with a graphics-state `cm` matrix that pivots around the placement centre. DOCX backend ignores the transform and renders the un-transformed geometry — Apache POI cannot express graphics-state matrices, and authors who need rotated output must export to PDF. |
