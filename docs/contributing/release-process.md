...
5c. **Regenerate and verify the knowledge pack** — `node knowledge/tools/api-surface/extract-api.mjs --from-reactor` rewrites `knowledge/api/*.json|md` and `knowledge/manifest.json` from the classes the verify just compiled, so the surfaces embed the just-bumped version (the extractor reads it from the root `pom.xml`). Then the script runs the same claims, routing, and self-sufficient bundle gates as [`release.yml`](../../.github/workflows/release.yml), including `node knowledge/tools/bundle/build-bundle.mjs --verify`. A stale index, broken route, or incomplete archive therefore fails before commit/tag. **Not** skipped by `-SkipVerify` (Step 4's installs already compiled every module the extractor reads). Node.js is a Step-0 prerequisite whenever `knowledge/` exists and a version bump will run; if `node` is not on `PATH`, both the full cut and a bump-performing `-PostReleaseOnly` abort before changing files.
...
7. **Open the next development line** — `pwsh ./scripts/cut-release.ps1 -PostReleaseOnly`. This bumps the train poms to the next minor `-SNAPSHOT` (so develop builds are distinguishable from the release and the japicmp gate compares against it) **and** restores linkable "View Code" buttons by flipping ShowcaseMetadata back to `/blob/develop`. The README/showcase install snippets stay on the just-published release.
...
> **Post-GA `-SNAPSHOT` naming.** `develop` always carries the next *minor*
> `-SNAPSHOT`; patch releases are cut from a dedicated release line. Accordingly,
> `-PostReleaseOnly` maps `X.Y.Z` to `X.(Y+1).0-SNAPSHOT` so the in-flight version
> matches `@since` tags and the open CHANGELOG heading without a manual correction.
