...
2. **Bump versions** to `<X.Y.Z>` across all 13 train poms (§0.D), **and** the install snippets — every train coordinate in the root `README.md` (`graph-compose` and any module the page tells a reader to add beside it, such as `graph-compose-testing`), then every per-module README and the listed documentation pages through a separate pass (`Update-ModuleReadmeInstallVersion`). Both passes skip `graph-compose-fonts` and `graph-compose-emoji`. All in the same step, so `VersionConsistencyGuardTest` stays green at Step 5. (`fonts/pom.xml` and `emoji/pom.xml` are left alone — they version independently; see §2.D.)
   ...
   The published site is bumped here too, by `Update-SiteReleaseData`. `web/index.html` states the version in seven places that inherit from no pom: the `release-context` JSON block the page itself reads (`stableVersion`, `releaseTag`), the JSON-LD `softwareVersion` and Maven Central `downloadUrl` a crawler reads, and the hero badge and the Maven + Gradle snippets a visitor reads. **The page is generated**, so none of them is edited in place: the cut moves the two values in `web-src/data/release.json` — the bare semver and the v-prefixed tag — and Step 4d rebuilds the pages with `node scripts/site/build.mjs`, which injects them into all seven. The rebuild sits after Step 4c deliberately: the pages are generated from `web/examples.json` as well, and 4c regenerates that, so building any earlier would render them from a catalogue the cut then replaces. Editing the page instead would be undone by the next build, and the site's own drift check would fail on the release commit. A pattern that matches nothing **stops the cut**, naming the spot, rather than leaving it on the previous release while reporting success. `VersionConsistencyGuardTest` still holds every one of those seven occurrences *in the built page*, and two steps in [`release-script-check.yml`](../../.github/workflows/release-script-check.yml) cover the halves: **Unit-check the site release-data bump** runs the function on a fixture (both values move, a non-version field stays put, a second run is a no-op, a renamed spot refuses), and **The published page is rebuilt from the bumped data, not edited** runs the real build on bumped data and asserts the version reaches all seven spots.
...
| `cut-release.ps1` stops at Step 6 or 7 with `git commit … failed` or `git tag … failed` and `Unable to create '.git/index.lock': File exists` | Another git process — an IDE, another agent working in the same tree — held the index, or left a stale lock behind | Nothing was pushed. A Step 6 stop committed nothing; a Step 7 stop left the `Release v<X.Y.Z>` commit without a tag. Make sure no git process is running and delete `.git/index.lock`, then finish the remaining steps by hand with the file list and messages the script prints under `-DryRun`, or reset to `origin/<branch>` and re-run the cut |
