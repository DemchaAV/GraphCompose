...
2. **Bump versions** to `<X.Y.Z>` across all 13 train poms (§0.D), **and** the install snippets — every train coordinate in the root `README.md` (`graph-compose` and any module the page tells a reader to add beside it, such as `graph-compose-testing`), then every per-module README and the listed documentation pages through a separate pass (`Update-ModuleReadmeInstallVersion`). Both passes skip `graph-compose-fonts` and `graph-compose-emoji`. All in the same step, so `VersionConsistencyGuardTest` stays green at Step 5. (`fonts/pom.xml` and `emoji/pom.xml` are left alone — they version independently; see §2.D.)
   ...
   The published site is bumped here too, by `Update-IndexHtmlVersion`. `web/index.html` states the version in seven places that inherit from no pom: the `release-context` JSON block the page itself reads (`stableVersion`, `releaseTag`), the JSON-LD `softwareVersion` and Maven Central `downloadUrl` a crawler reads, and the hero badge and the Maven + Gradle snippets a visitor reads. **Every occurrence of each moves**, not the first — a first-match rewrite is how a page ends up with a second snippet a release behind — and a pattern that matches nothing **stops the cut**, naming the spot, rather than leaving it on the previous release while reporting success. `VersionConsistencyGuardTest` holds every one of those occurrences to the release, and the **Unit-check the showcase version bump** step in [`release-script-check.yml`](../../.github/workflows/release-script-check.yml) runs the function on a fixture: every token moves, a second run is a no-op, and a renamed spot refuses.
