...
Four scripts gained a covering family in fonts **1.1.0**: `FontName.AMIRI`
for Arabic, `FontName.DAVID_LIBRE` for Hebrew, `FontName.NOTO_SANS_GEORGIAN` and
`FontName.NOTO_SANS_ARMENIAN`. Each carries its own script plus Latin and none of
the other three, and a paragraph is drawn in a single family, so a run mixing two
of them needs a font of your own. The full table is in the
