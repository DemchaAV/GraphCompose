...
Page backgrounds (`session.pageBackgrounds(...)` — a full-page tint, a sidebar column, a
band) are painted behind the text of every page. Word colours a page in one colour only, so
...
- **Text at the left and the right of one band → one line with a right tab.** A
  ...
  lifts the table by that much out of the gap above it, and the row's other cells start
  that much lower inside. Each paragraph
