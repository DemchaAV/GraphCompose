...
Copies the *reachable* noto colour-emoji SVGs (those a gemoji shortcode maps to) into
the module under a codepoint filename and generates the shortcode index that
EmojiLibrary resolves. Noto glyphs no shortcode points at (mostly skin-tone variants)
are skipped — EmojiLibrary resolves via the index only, so they would just be dead
weight in the published artifact.
...
def main() -> int:
    ...
    # 1) Index every noto glyph by codepoint key (no copy yet — see step 3).
    available = {}
    for src in sorted(noto_dir.glob("emoji_u*.svg")):
        cps = noto_codepoints(src.name)
        if not cps:
            continue
        available["-".join(cps)] = src
    keys = set(available)

    # 2) Build the shortcode index from gemoji, matching codepoints to available glyphs.
    ...
    # 3) Copy only the glyphs an index shortcode can actually resolve.
    if svg_out.exists():
        shutil.rmtree(svg_out)
    svg_out.mkdir(parents=True, exist_ok=True)
    reachable = sorted(set(index.values()))
    for key in reachable:
        shutil.copyfile(available[key], svg_out / f"{key}.svg")
    glyphs = len(reachable)

    # 4) Write the index, sorted for a stable diff.
    ...
    print(f"glyphs reachable & copied   : {glyphs}")
    print(f"glyphs skipped (unreachable): {len(available) - glyphs}")
