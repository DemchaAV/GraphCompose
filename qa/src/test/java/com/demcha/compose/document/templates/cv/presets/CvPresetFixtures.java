...
/**
 * Shared roster and canonical sample document for the layered CV
 * presets.
 *
 * <p>Both preset gates read from here so they always describe the same
 * render: {@code CvV2VisualParityTest} compares the rasterised pages
 * per-pixel, {@code CvPresetLayoutSnapshotTest} compares the
 * post-layout node tree. Adding a preset to {@link #presets()} enrols
 * it in both.</p>
 *
 * <p>The two gates are complementary rather than redundant. The pixel
 * budget has to absorb cross-platform PDFBox font rendering drift, so
 * it is deliberately loose (50 000 mismatched pixels) and cannot see a
 * structural change that happens to paint a similar picture — a body
 * that switches from a row to a column flow, a node that gains a
 * parent, geometry that shifts by a point. The layout snapshot is an
 * exact comparison of computed geometry and node identity, so it
 * catches exactly that class of drift.</p>
 */
final class CvPresetFixtures {
    ...
    /**
     * Every layered CV preset, as {@code (slug, recommendedMargin,
     * factory)} triples.
     */
     ...
     * subtype exercised so the gates cover paragraph, grouped skills,
