...
/**
 * Pixel-diff visual parity gate for {@link MidnightNavy} — the canonical
 ...
 */
class MidnightNavyVisualParityTest {
    ...
    void rendersWithinPixelDiffBudget() throws Exception {
        ...
        try (DocumentSession document = GraphCompose.document().create()) {
            MidnightNavy.create().compose(document, MidnightNavyFixtures.canonicalCv());
        ...
        PdfVisualRegression.standard()
                ...
                .assertMatchesBaseline("midnight_navy", pdfBytes);
    }
}
