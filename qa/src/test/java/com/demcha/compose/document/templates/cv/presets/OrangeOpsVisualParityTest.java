...
/**
 * Pixel-diff visual parity gate for {@link OrangeOps} — the canonical one-page
 * CV against a checked-in baseline.
 *
 ...
 * PNG with the change. Baselines live under
 * {@code src/test/resources/visual-baselines/cv-v2-layered/}.</p>
 */
class OrangeOpsVisualParityTest {
    ...
    void rendersWithinPixelDiffBudget() throws Exception {
        ...
        try (DocumentSession document = GraphCompose.document().create()) {
            OrangeOpsTestFont.register(document);
            OrangeOps.create().compose(document, OrangeOpsFixtures.canonicalCv());
        ...
        PdfVisualRegression.standard()
                ...
                .assertMatchesBaseline("orange_ops", pdfBytes);
    }
}
