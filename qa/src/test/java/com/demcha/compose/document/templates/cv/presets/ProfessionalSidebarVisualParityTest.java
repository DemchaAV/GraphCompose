...
/**
 * Pixel-diff visual parity gate for {@link ProfessionalSidebar} — the
 * canonical one-page CV against a checked-in baseline.
 *
    ...
    void rendersWithinPixelDiffBudget() throws Exception {
        ...
        try (DocumentSession document = GraphCompose.document().create()) {
            ProfessionalSidebar.create().compose(
                    document, ProfessionalSidebarFixtures.canonicalCv());
        ...
        PdfVisualRegression.standard()
                ...
                .assertMatchesBaseline("professional_sidebar", pdfBytes);
    }
}
