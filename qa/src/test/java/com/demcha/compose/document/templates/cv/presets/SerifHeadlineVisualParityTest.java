...
/**
 * Pixel-diff visual parity gate for {@link SerifHeadline} - the canonical
 ...
 */
class SerifHeadlineVisualParityTest {
    ...
    void rendersWithinPixelDiffBudget() throws Exception {
        ...
        try (DocumentSession document = GraphCompose.document().create()) {
            SerifHeadline.create().compose(document, SerifHeadlineFixtures.canonicalCv());
        ...
        PdfVisualRegression.standard()
                ...
                .assertMatchesBaseline("serif_headline", pdfBytes);
    }
}
