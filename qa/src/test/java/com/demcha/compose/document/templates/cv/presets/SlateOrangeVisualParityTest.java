...
/**
 * Pixel-diff visual parity gate for {@link SlateOrange} — the canonical
 ...
 */
class SlateOrangeVisualParityTest {
    ...
    void rendersWithinPixelDiffBudget() throws Exception {
        ...
        try (DocumentSession document = GraphCompose.document().create()) {
            SlateOrange.create().compose(document, SlateOrangeFixtures.canonicalCv());
        ...
        PdfVisualRegression.standard()
                ...
                .assertMatchesBaseline("slate_orange", pdfBytes);
    }
}
