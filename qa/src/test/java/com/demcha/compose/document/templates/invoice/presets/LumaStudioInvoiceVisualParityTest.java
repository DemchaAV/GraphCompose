...
/**
 * Pixel-diff visual parity gate for {@link LumaStudioInvoice}. Budget and
 * tolerance mirror the other structured-invoice parity gates.
 *
 ...
 */
class LumaStudioInvoiceVisualParityTest {
    ...
    void rendersWithinPixelDiffBudget() throws Exception {
        ...
        try (DocumentSession document = GraphCompose.document().create()) {
            LumaStudioInvoice.create().compose(
                    document, LumaStudioInvoiceFixtures.canonicalInvoice());
        ...
        PdfVisualRegression.standard()
                ...
                .assertMatchesBaseline("luma_studio_invoice", pdfBytes);
    }
}
