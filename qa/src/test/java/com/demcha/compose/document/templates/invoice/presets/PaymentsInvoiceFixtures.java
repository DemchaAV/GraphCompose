...
/**
 * Shared fixture data for the {@link PaymentsInvoice} gates — the SAME document
 * feeds the pixel parity test and the layout snapshot test, so a geometry shift
 * the pixel budget absorbs still trips the exact snapshot, and vice versa.
 */
final class PaymentsInvoiceFixtures {

    private PaymentsInvoiceFixtures() {
    }

    /**
     * The canonical one-page invoice — seven metadata rows, two addressed
     * parties, six marked service lines, seven payment fields, two summed rows
     * and a two-line note.
     *
     * @return the document
     */
