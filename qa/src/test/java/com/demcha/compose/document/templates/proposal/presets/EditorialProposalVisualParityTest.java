...
/**
 * Pixel-diff visual parity gate for {@link EditorialProposal} — both pages
 ...
 * the updated PNGs with the change. Baselines live under
 * {@code src/test/resources/visual-baselines/proposal-v2-layered/}.</p>
 */
class EditorialProposalVisualParityTest {
    ...
    void rendersWithinPixelDiffBudget() throws Exception {
        ...
        try (DocumentSession document = GraphCompose.document().create()) {
            EditorialProposal.create().compose(
                    document, EditorialProposalFixtures.canonicalProposal());
        ...
        PdfVisualRegression.standard()
                ...
                .assertMatchesBaseline("editorial_proposal", pdfBytes);
    }
}
