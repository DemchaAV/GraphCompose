...
import com.demcha.components.data.text.TextBlockMeasurer;
 ...
 * </ul>
 *
 * <p>This class makes use of the {@link Size}, {@link Position}, and {@link Margin} components
...
public class HorizontalLayout implements Layout {
    ...
    public void measure(Container c, MeasureCtx ctx) {
        ...
        c.getElement().getOrAdd(Size.class, () -> new Size(totalWfinal, maxHfinal));
    }
    ...
    public void arrange(Container c, ArrangeCtx ctx) {
        ...
        double containerH = ctx.allocatedHeight() > 0
                ? ctx.allocatedHeight()
                : c.getElement().get(Size.class).map(Size::height).orElse(0.0);
