...
public class InvoiceTemplateV1 implements InvoiceTemplate {
    ...
    private Entity createHeaderLeft(ComponentBuilder cb, InvoiceData data, double width) {
        ...
        column.addChild(createText(cb, data.title(), BusinessDocumentStyles.titleStyle(TITLE_SIZE), Anchor.topLeft(), Margin.zero()));
        column.addChild(createText(cb, "Invoice #" + valueOrFallback(data.invoiceNumber(), "Draft"),
                BusinessDocumentStyles.headingStyle(SUBTITLE_SIZE + 1), Anchor.topLeft(), Margin.zero()));

        if (!data.status().isBlank()) {
            column.addChild(createText(cb, "Status: " + data.status(), BusinessDocumentStyles.labelStyle(LABEL_SIZE),
                    Anchor.topLeft(), Margin.zero()));
        }

        if (!data.reference().isBlank()) {
            column.addChild(createText(cb, "Reference: " + data.reference(), BusinessDocumentStyles.metaStyle(META_SIZE),
                    Anchor.topLeft(), Margin.zero()));
        }
    ...
    private Entity createPartyColumn(ComponentBuilder cb, String title, InvoiceParty party, double width) {
        ...
        column.addChild(createText(cb, title, BusinessDocumentStyles.labelStyle(LABEL_SIZE), Anchor.topLeft(), Margin.top(2)));
        column.addChild(createText(cb, valueOrFallback(safeParty.name(), "Not provided"), BusinessDocumentStyles.bodyBoldStyle(BODY_SIZE + 1),
                Anchor.topLeft(), Margin.zero()));
    ...
    private Entity createItemsTable(ComponentBuilder cb, InvoiceData data, double width) {
        TableBuilder table = cb.table()
                ...
                .defaultCellStyle(TableCellStyle.builder()
                        ...
                        .stroke(new Stroke(BusinessDocumentStyles.BORDER_COLOR, 1.3))
                        .textStyle(BusinessDocumentStyles.bodyStyle(9.3))
                        .textAnchor(Anchor.centerLeft())
                        .build())
                .rowStyle(0, TableCellStyle.builder()
                        .fillColor(BusinessDocumentStyles.STRONG_FILL)
                        .textStyle(BusinessDocumentStyles.headingStyle(9.0))
                        .build())
    ...
    private Entity createSummaryColumn(ComponentBuilder cb, InvoiceData data, double width) {
        ...
        TableBuilder table = cb.table()
                ...
                .defaultCellStyle(TableCellStyle.builder()
                        ...
                        .fillColor(BusinessDocumentStyles.SOFT_FILL)
                        .stroke(new Stroke(BusinessDocumentStyles.BORDER_COLOR, 1.3))
                        .textStyle(BusinessDocumentStyles.bodyStyle(9.4))
                        .textAnchor(Anchor.centerLeft())
                        .build())
                .columnStyle(1, TableCellStyle.builder()
                        .textAnchor(Anchor.centerRight())
        ...
        for (int index = 0; index < rows.size(); index++) {
            ...
            if (row.emphasized()) {
                table.rowStyle(index, TableCellStyle.builder()
                        .fillColor(BusinessDocumentStyles.STRONG_FILL)
                        .textStyle(BusinessDocumentStyles.bodyBoldStyle(9.6))
                        .build());
            }
        }
    ...
    private Entity createMetaLine(ComponentBuilder cb, String label, String value, double width) {
        Entity left = createText(cb, label, BusinessDocumentStyles.labelStyle(LABEL_SIZE), Anchor.topLeft(), Margin.zero());
        Entity right = createText(cb, value, BusinessDocumentStyles.bodyBoldStyle(META_SIZE), Anchor.topRight(), Margin.zero());
    ...
    private Entity createSectionHeader(ComponentBuilder cb, String value, double width, Margin margin) {
        VContainerBuilder header = cb.vContainer(Align.left(1))
                ...
                .anchor(Anchor.topLeft())
                .margin(margin);
        header.addChild(createText(cb, value, BusinessDocumentStyles.labelStyle(LABEL_SIZE), Anchor.topLeft(), Margin.zero()));
    ...
    private Entity createRule(ComponentBuilder cb, double width, Margin margin) {
        return cb.line()
                ...
                .stroke(new Stroke(BusinessDocumentStyles.ACCENT_COLOR, RULE_STROKE))
                .anchor(Anchor.topLeft())
                .margin(margin)
                .build();
    }

    private Entity createParagraph(ComponentBuilder cb, List<String> paragraphs, double width, Margin margin, double fontSize, String entityName) {
        List<String> sanitized = sanitizeLines(paragraphs);
        BlockTextBuilder builder = cb.blockText(Align.left(2), BusinessDocumentStyles.bodyStyle(fontSize))
                ...
                .text(sanitized, BusinessDocumentStyles.bodyStyle(fontSize), Padding.zero(), Margin.zero());
    ...
    private Entity createBulletParagraph(ComponentBuilder cb, List<String> items, double width, Margin margin, String entityName) {
        ...
        BlockTextBuilder builder = cb.blockText(Align.left(2), BusinessDocumentStyles.bodyStyle(BODY_SIZE))
                .entityName(entityName)
                .size(width, 2)
                ...
                .margin(margin)
                .padding(Padding.zero())
                .text(sanitized, BusinessDocumentStyles.bodyStyle(BODY_SIZE), Padding.zero(), Margin.zero());
        return builder.build();
    }
