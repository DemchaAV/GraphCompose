...
public class ProposalTemplateV1 implements ProposalTemplate {
    ...
    private Entity createHeader(ComponentBuilder cb, ProposalData data, double width) {
        ...
        root.addChild(createText(cb, data.title(), BusinessDocumentStyles.titleStyle(TITLE_SIZE), Anchor.topLeft(), Margin.zero()));
        root.addChild(createText(cb, valueOrFallback(data.projectTitle(), "Project proposal"),
                BusinessDocumentStyles.headingStyle(13), Anchor.topLeft(), Margin.zero()));
    ...
    private Entity createPartyColumn(ComponentBuilder cb, String title, ProposalParty party, double width) {
        ...
        column.addChild(createText(cb, title, BusinessDocumentStyles.labelStyle(LABEL_SIZE), Anchor.topLeft(), Margin.zero()));
        if (!safeParty.name().isBlank()) {
            column.addChild(createText(cb, safeParty.name(), BusinessDocumentStyles.bodyBoldStyle(BODY_SIZE + 0.7),
    ...
    private Entity createParagraph(ComponentBuilder cb, List<String> paragraphs, double width, Margin margin,
            double fontSize, String entityName) {
