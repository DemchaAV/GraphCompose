...
/**
 * Smoke test for the v2 Sidebar Portrait preset. Covers the
 * portrait + contact stack, the multi-section sidebar (education /
 * skills / languages), and the hero name strip + main column body.
 */
class SidebarPortraitSmokeTest {

    @Test
    void exposes_stable_identity() {
        DocumentTemplate<CvDocument> template = SidebarPortrait.create();
        assertThat(template.id()).isEqualTo("sidebar-portrait");
        assertThat(template.displayName()).isEqualTo("Sidebar Portrait");
    }

    @Test
    void default_factory_renders_full_document() throws Exception {
        renderAndAssertNonEmpty(SidebarPortrait.create(), fullDocument());
    }

    @Test
    void custom_theme_factory_renders() throws Exception {
        renderAndAssertNonEmpty(SidebarPortrait.create(BrandTheme.sidebarPortrait()),
                fullDocument());
    }
