...
/**
 * Smoke test for the v2 Timeline Minimal preset. Covers the icon
 * contact stack and section-line flattening across every
 * {@code CvSection} subtype.
 */
class TimelineMinimalSmokeTest {

    @Test
    void exposes_stable_identity() {
        DocumentTemplate<CvDocument> template = TimelineMinimal.create();
        assertThat(template.id()).isEqualTo("timeline-minimal");
        assertThat(template.displayName()).isEqualTo("Timeline Minimal");
    }

    @Test
    void default_factory_renders_full_document() throws Exception {
        renderAndAssertNonEmpty(TimelineMinimal.create(), fullDocument());
    }

    @Test
    void custom_theme_factory_renders() throws Exception {
        renderAndAssertNonEmpty(TimelineMinimal.create(BrandTheme.timelineMinimal()),
                fullDocument());
    }
