...
class EngineComposerHarnessLayoutSnapshotTest {
    ...
    private void composeBlockTextProbe(EngineComposerHarness composer) {
    ...
    private void composeLayerProbe(EngineComposerHarness composer) {
        ComponentBuilder cb = composer.componentBuilder();
    ...
    private double firstLineX(EngineComposerHarness composer, String entityName) throws Exception {
        return firstLine(composer, entityName).x();
    }

    private double firstLineWidth(EngineComposerHarness composer, String entityName) throws Exception {
        return firstLine(composer, entityName).lineWidth();
    }

    private com.demcha.compose.layout_core.components.content.text.LineTextData firstLine(EngineComposerHarness composer, String entityName) throws Exception {
        EntityManager entityManager = entityManager(composer);
    ...
    private EntityManager entityManager(EngineComposerHarness composer) throws Exception {
        Field entityManagerField = EngineComposerHarness.class.getDeclaredField("entityManager");
        entityManagerField.setAccessible(true);
        return (EntityManager) entityManagerField.get(composer);
    }

    private LayoutSystem<?> layoutSystem(EngineComposerHarness composer) throws Exception {
        Field layoutSystemField = EngineComposerHarness.class.getDeclaredField("layoutSystem");
