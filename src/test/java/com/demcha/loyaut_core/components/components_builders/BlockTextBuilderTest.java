...
class BlockTextBuilderTest {
    ...
    void testComplexMarkdownText() {
        ...
        // Since the input is a single string with newlines, we can split it or pass as list
        List<String> textList = List.of(text);
        
        builder.text(textList, textStyle, Padding.zero(), Margin.zero(), null);
        ...
        // Verify that bold/italic parsing happened (checking if we have different styles in the first line)
        // The first line starts with "**Lead Engineer**", so the first segment should be bold.
        var firstLine = blockTextData.lines().get(0);
        assertFalse(firstLine.bodies().isEmpty());
        
        // Note: Exact verification depends on how MarkDownParser splits tokens and how line breaking works with the font.
