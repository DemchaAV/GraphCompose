 ...
 * specific drawn sheet — {@code ProfessionalSidebar} is the one here — takes
 * no {@code BrandTheme} and calls no shared renderer: its geometry, palette
 * and type scale are measured constants of its own, and it sets its own page.
 * The data model is the same {@code CvDocument}; what differs is that the
 * cosmetics cannot be swapped, because moving them would stop it being the
 * sheet it reproduces.</p>
 */
